...
namespace BLECode
{
    public class BluetoothLECode
    {
        ...
        public List<BluetoothLEDevice> DeviceList { get; set; }
            ...
            BluetoothLEDevice.FromBluetoothAddressAsync(args.BluetoothAddress).Completed = (asyncInfo, asyncStatus) =>
            {
                if (asyncStatus == AsyncStatus.Completed)
                {
                    if (asyncInfo.GetResults() != null)
                    {
                        ...
                        //    ValueChanged(MsgType.NotifyTxt, "发现设备：" + currentDevice.Name + "####" + currentDevice.DeviceInformation.Id);
                        //}
                        ValueChanged(MsgType.NotifyTxt, "发现设备：" + currentDevice.Name + "####" + currentDevice.DeviceInformation.Id);
                    }
                }
        ...
        {
            ValueChanged(MsgType.NotifyTxt, "发现设备:" + args.Name+ "   " + args.Id);
        ...
        /// <param name="MAC"></param>
        /// <returns></returns>
        public async Task SelectDeviceFromMacAsync(string MAC)
        {
            CurrentDeviceMAC = MAC;
            CurrentDevice = null;
        ...
        public async Task SelectDeviceFromIdAsync(string Id)
        {
            BluetoothAdapter.GetDefaultAsync().Completed = async (asyncInfo, asyncStatus) =>
            {
                if (asyncStatus == AsyncStatus.Completed)
        ...
        private async Task Connect()
        {
            ...
            //{
            //    ValueChanged(MsgType.NotifyTxt, "Service UUID:" + service.Uuid.ToString());
            //    foreach (var characteristic in service.GetAllCharacteristics())
            //    {
            //        ValueChanged(MsgType.NotifyTxt, "Characteristic UUID:" + characteristic.Uuid.ToString());
            //    }
            //}
            //CurrentDevice.GetGattServicesAsync().Completed = (result, asyncStatus) => 
            //{
            //    if(asyncStatus == AsyncStatus.Completed)
            //    {
            //        foreach (var service in result.GetResults().Services)
            //        {
            //            ServiceGuid = service.Uuid.ToString();
            //            ValueChanged(MsgType.NotifyTxt, "Service UUID:" + service.Uuid.ToString());
            //            service.GetCharacteristicsAsync().Completed = (result2, asyncStatus2) =>
            //            {
            //                if(asyncStatus2 == AsyncStatus.Completed)
            //                {
            //                    foreach(var characteristic in result2.GetResults().Characteristics)
            //                    {
            //                        ValueChanged(MsgType.NotifyTxt, "Characteristic UUID:" + characteristic.Uuid.ToString());
            //                    };
            //                }
        ...
        private void Characteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args)
        {
            ...
            ValueChanged(MsgType.BLEData, sender.Service.Device.Name, data);
        }
    }
